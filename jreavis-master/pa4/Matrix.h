...
void freeMatrix(Matrix *M); //Frees heap memory associated with *M, sets *M to NULL
...
void printMatrix(FILE *out, Matrix M);//Prints a string representation of Matrix M to filestream out. Zero rows are not printed.
                                      //Each non-zero row is represented as one line consisting of the row number, followed by a 
                                      //colon, a space, then a space separated list of pairs "(col, val)" giving the column numbers
                                      //and non-zero in that row. The double val will be rounded to 1 decimal point
