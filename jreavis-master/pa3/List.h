...
void freeList(List *L); //Frees all heap memory associated with *pL, and sets *pL to NULL
...
void movePrev(List L); //If cursor is defined and not at front, move cursor one step towards the front of L; if cursor is
                       //defined and at front, cursor becomes undefined; if cursor is undefined do nothing

void moveNext(List L); //If cursor is defined and not at back, move cursor one step towards the back of L; if cursor is
...
//Other operations
void printList(FILE* out, List L); //Prints to the file pointed to by out, a string representation of L consisting of
                                   //a space separated sequence of integers, with front on the left
            
List copyList(List L); //Returns a new List representing the same integer sequence as L. The cursor in the new list is
                       //undefined, regardless of the state of the cursor in L. The state of L is unchanged

//Optional operations
List concatList(List A, List B); //Returns a new list which is the concatenation of A and B. The cursor in the new list is
                                 //undefined, regardless of the states of the cursors in A and B. The states of A and B are unchanged
#endif
